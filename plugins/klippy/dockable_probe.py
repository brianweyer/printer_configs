...
class DockableProbe:
    ...
    def _move_to_vector(self, angle):
        # self.gcode.respond_info('dockable_probe._move_to_vector(%r)' % (angle,))
    ...
    def _align_to_vector(self, angle):
        # self.gcode.respond_info('dockable_probe._align_to_vector(%r)' % (angle,))
    ...
    def _align_z(self):
        # self.gcode.respond_info('dockable_probe._align_z()')
    ...
    def _force_z_hop(self):
        # self.gcode.respond_info('dockable_probe._force_z_hop()')
    ...
    def attach_probe(self, return_pos=None):
        # self.gcode.respond_info('dockable_probe.attach_probe(return_pos=%r)' % (return_pos,))
        ...
        if return_pos:
            if not self._check_distance(return_pos, self.approach_distance):
                self.toolhead.manual_move(
                    ...
                    self.travel_speed)
    def _do_attach(self):
        # self.gcode.respond_info('dockable_probe._do_attach()')
        if self.get_probe_state() != PROBE_DOCKED:
            raise self.printer.command_error(
    ...
    def detach_probe(self, return_pos=None):
        # self.gcode.respond_info('dockable_probe.detach_probe(return_pos=%r)' % (return_pos,))
        if self.get_probe_state() == PROBE_DOCKED:
            return
        ...
        if return_pos:
            if not self._check_distance(return_pos, self.detach_distance):
                self.toolhead.manual_move(
                    ...
                    self.travel_speed)
    def _do_detach(self):
        # self.gcode.respond_info('dockable_probe._do_detach()')
        if self._check_distance(dist=self.detach_distance):
            self._align_to_vector(self.dock_angle)
        else:
            self.toolhead.manual_move(
    ...
    def _handle_homing_rails_begin(self, homing_state, rails):
        # self.gcode.respond_info('dockable_probe._handle_homing_rails_begin(homing_state=%r, rails=%r)' % (homing_state, rails))
        ...
        # self.gcode.respond_info('dockable_probe._handle_homing_rails_begin homing_axes=%r is_z_endstop=%r' % (homing_axes, self.is_z_endstop))
    ...
    def _handle_homing_rails_end(self, homing_state, rails):
        # self.gcode.respond_info('dockable_probe._handle_homing_rails_end(homing_state=%r, rails=%r)' % (homing_state, rails))
        self.is_z_endstop = False
        self._last_homed = None
    ...
    def multi_probe_begin(self):
        # self.gcode.respond_info('dockable_probe.multi_probe_begin()')
    ...
    def multi_probe_end(self):
        # self.gcode.respond_info('dockable_probe.multi_probe_end()')
        self.multi = MULTI_OFF
        pos = self.toolhead.get_position()
    ...
    def probe_prepare(self, hmove):
        # self.gcode.respond_info('dockable_probe.probe_prepare(hmove=%r)' % (hmove,))
        if self.multi == MULTI_FIRST:
            self.multi = MULTI_ON
    def probe_finish(self, hmove):
        # self.gcode.respond_info('dockable_probe.probe_finish(hmove=%r)' % (hmove,))
        self.wait_trigger_complete.wait()
    def home_start(self, print_time, sample_time, sample_count, rest_time,
                   triggered=True):
        # self.gcode.respond_info('dockable_probe.home_start(print_time=%r, sample_time=%r, sample_count=%r, rest_time=%r, triggered=%r)' % (print_time, sample_time, sample_count, rest_time, triggered))
